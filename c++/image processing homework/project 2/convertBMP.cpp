...
void ReadLine(vector<Pixel> *Vec, FILE *Src)
{
    ...
    for (i = BMPInfoHeader.biWidth * 3; i % 4 != 0; i++)
    {
        unsigned char buf;
...
vector<Pixel> v1, v2, v3, u1, u2, u3;
vector<Pixel> *VecMap[3];
vector<Pixel> *VecMap2[3];
...
int main()
{
    ...
    if (BMP)
    {
        ...
        for (int i = 0; i < 3; i++)
            (*VecMap[i]).clear();
        // Then, apply dialation to Output1, and erosion to Output2, to realize opening and closing.
        FILE *Op1 = fopen(".\\Output1.bmp", "rb");
        FILE *Op2 = fopen(".\\Output2.bmp", "rb");
        FILE *Op3 = fopen(".\\Output3.bmp", "wb+");
        FILE *Op4 = fopen(".\\Output4.bmp", "wb+");
        // Since headers are same, there is no need to read headers.
        for (int i = 1; i <= BMPHeader.bfOffBits; i++)
        {
            unsigned char buf;
            fread(&buf, sizeof(char), 1, Op1);
            fread(&buf, sizeof(char), 1, Op2);
            fwrite(&buf, sizeof(char), 1, Op3);
            fwrite(&buf, sizeof(char), 1, Op4);
        }
        // Set vector map
        VecMap[0] = &v1;
        VecMap[1] = &v2;
        ...
        VecMap2[2] = &u3;
        // Read two lines.
        ReadLine(VecMap[0], Op1);
        ReadLine(VecMap2[0], Op2);
        ReadLine(VecMap[1], Op1);
        ReadLine(VecMap2[1], Op2);
        // Output First Line without modifying.
        for (int i = 0; i < BMPInfoHeader.biWidth; i++)
        {
            fwrite((*VecMap[0])[i].Bytes, sizeof(char), 3, Op3);
            fwrite((*VecMap2[0])[i].Bytes, sizeof(char), 3, Op4);
        }
        for (int i = BMPInfoHeader.biWidth * 3; i % 4 != 0; i++)
        {
            unsigned char buf = 0;
        ...
        // begin dilation and erosion process
        for (int i = 0; i < BMPInfoHeader.biHeight - 2; i++)
        {
            // Read a line
            ReadLine(VecMap[2], Op1);
            ReadLine(VecMap2[2], Op2);
            // Output first element without modifying
            fwrite((*VecMap[1])[0].Bytes, sizeof(char), 3, Op3);
            fwrite((*VecMap2[1])[0].Bytes, sizeof(char), 3, Op4);
            // process
            for (int j = 1; j < BMPInfoHeader.biWidth - 1; j++)
            {
                int flag1 = 0, flag2 = 0;
                if ((*VecMap[0])[j].YUV[0] < 128 || (*VecMap[2])[j].YUV[0] < 128)
                    flag1 = 1;
                if ((*VecMap[1])[j - 1].YUV[0] < 128 || (*VecMap[1])[j + 1].YUV[0] < 128)
                    flag1 = 1;
                if ((*VecMap[1])[j].YUV[0] < 128)
                    flag1 = 1;
                if ((*VecMap2[0])[j].YUV[0] >= 128 || (*VecMap2[2])[j].YUV[0] >= 128)
                    flag2 = 1;
                if ((*VecMap2[1])[j - 1].YUV[0] >= 128 || (*VecMap2[1])[j + 1].YUV[0] >= 128)
                    flag2 = 1;
                if ((*VecMap2[1])[j].YUV[0] >= 128)
                    flag2 = 1;
                Pixel pixel;
                pixel.YUV[0] = flag1 ? 0 : 255;
                pixel.YUV[1] = pixel.YUV[2] = 0;
                pixel.ConvertRGB();
                swap(&pixel.Bytes[0], &pixel.Bytes[2]);
                fwrite(pixel.Bytes, sizeof(char), 3, Op3);
                pixel.YUV[0] = flag2 ? 255 : 0;
                pixel.YUV[1] = pixel.YUV[2] = 0;
                pixel.ConvertRGB();
                swap(&pixel.Bytes[0], &pixel.Bytes[2]);
                fwrite(pixel.Bytes, sizeof(char), 3, Op4);
            }
            // Output last element without modifying
            fwrite((*VecMap[1]).back().Bytes, sizeof(char), 3, Op3);
            fwrite((*VecMap2[1]).back().Bytes, sizeof(char), 3, Op4);
            // end space
            for (int j = BMPInfoHeader.biWidth * 3; j % 4 != 0; j++)
            {
                unsigned char buf = 0;
        ...
        // Output Last Line without modifying.
        for (int i = 0; i < BMPInfoHeader.biWidth; i++)
        {
            fwrite((*VecMap[2])[i].Bytes, sizeof(char), 3, Op3);
            fwrite((*VecMap2[2])[i].Bytes, sizeof(char), 3, Op4);
        }
        for (int i = BMPInfoHeader.biWidth * 3; i % 4 != 0; i++)
        {
            unsigned char buf = 0;
